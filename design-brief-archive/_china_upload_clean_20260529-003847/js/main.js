...
async function renderSearchPage() {
  ...
  const records = [
    ...
    ...cases.map((item) => searchRecord("案例", item.title, `${item.competitionName} ${item.workType} ${item.caseHighlight} ${flattenText(item.analysis)}`, `case-detail.html?id=${item.id}`, item)),
    ...
    <section class="detail-grid">
      <article class="detail-block full">
        ...
        <dl class="field-list">
          ...
          ${field("来源深度", status.label)}
        </dl>
        <div class="card-actions">${sourceLink(item.sourceUrl, status.linkLabel)}</div>
      </article>
      ${detailStudyGuide(item, "case")}
      <article class="detail-block">
        <h2>案例亮点</h2>
        <p>${safe(item.caseHighlight)}</p>
      </article>
      ${caseAnalysisBlocks(item)}
      <article class="notice-card detail-block full">
        <h2>来源与版权提示</h2>
        <p>${safe(item.copyrightNote)}</p>
...
function sourceStatus(item) {
  ...
  const indexPatterns = [/\/websites\/?$/i, /\/awards?\/?$/i, /\/home\/?$/i, /index\.html$/i];
  const isSpecific = directPatterns.some((pattern) => pattern.test(url)) && !indexPatterns.some((pattern) => pattern.test(url)) && !title.includes("观察入口") && !title.includes("索引");
  return {
    label: isSpecific ? "具体作品页" : "官方合集入口",
    note: isSpecific ? "点击来源通常可以直接进入该作品档案。" : "当前只能跳到官方合集或赛事页面，后续需要继续补具体作品链接。",
    className: isSpecific ? "source-direct" : "source-index",
    linkLabel: isSpecific ? "打开具体作品" : "打开官方合集"
  };
}

function sourceBadge(item) {
  const status = sourceStatus(item);
  return `<span class="source-badge ${status.className}">${status.label}</span>`;
}

function detailStudyGuide(item, type) {
  const status = sourceStatus(item);
  const keywords = toArray(item.keywords).slice(0, 4).join(" / ");
  const title = type === "case" ? "这条案例怎么读" : "这件作品怎么读";
  return `<article class="detail-block full study-guide">
    <h2>${title}</h2>
    <div class="study-grid">
      <section>
        <h3>先看命题关系</h3>
        <p>不要只看画面好不好看，先判断作品如何回应比赛主题、目标人群和作品类型要求。</p>
      </section>
      <section>
        <h3>再看视觉系统</h3>
        <p>重点拆解标题、主视觉、辅助图形、色彩、字体、系列物料和展示顺序之间是否统一。</p>
      </section>
      <section>
        <h3>最后看可迁移方法</h3>
        <p>提炼调研路径、概念生成、版式组织和说明写法，不复制具体图像、素材或版式。</p>
      </section>
    </div>
    <p class="muted">关键词：${safe(keywords || "待补充")}。来源状态：${status.label}，${status.note}</p>
  </article>`;
}
...
function workCard(item) {
  ...
  return `<article class="work-card reveal">
    ...
    <div class="work-card-body">
      ...
      <p class="muted source-line">来源：${safe(item.sourceName)} ${sourceBadge(item)}</p>
      <div class="card-actions">
        ${favoriteButton("work", item.id)}
        ${sourceLink(item.sourceUrl, status.linkLabel)}
...
function caseCard(item) {
  ...
  return `<article class="work-card reveal">
    ...
    <div class="work-card-body">
      ...
      <p class="muted">${safe(caseAnalysisSummary(item))}</p>
    </div>
  </article>`;
}
...
function caseAnalysisSummary(item) {
  ...
  if (analysis && typeof analysis === "object" && !Array.isArray(analysis)) {
    return analysis.overview || analysis.readingPoints || flattenText(analysis);
  }
  return analysis || "待补充";
}
